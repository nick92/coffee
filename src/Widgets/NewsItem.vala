...
public class Coffee.Widgets.NewsItem : Gtk.FlowBoxChild {
    public Coffee.Post post { get; construct; }
    ...
    public NewsItem (Coffee.Post post) {
        Object (post: post);
    }
    ...
    construct {
        ...
        if(post.link != "")
          display_image = load_new_image (post.image_link);
        ...
        description_label.label = post.subject ?? "";
    ...
    private Gtk.Image load_new_image (string url) {
        ...
        Idle.add (() => {
            try {
                ...
                image.pixbuf = new Gdk.Pixbuf.from_file_at_scale (fileimage.get_path (), 300, 150, true);
                //image.show ();
            } catch (Error e) {
                critical (e.message);
            }
